...
public class EntertainmentAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        AllBean.ResultBean.BodyBean bean = allBean.getResult().get(15).getBody().get(i);
        EntertainmentHolder holder = null;
        if (view == null) {
            view = LayoutInflater.from(context).inflate(R.layout.item_recommend_music, viewGroup, false);
            holder = new EntertainmentHolder(view);
            view.setTag(holder);
        } else {
            holder = (EntertainmentHolder) view.getTag();
        }
